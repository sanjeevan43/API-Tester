...
export class SettingsProvider implements vscode.WebviewViewProvider {
  ...
  private onSettingsChanged: ((settings: any) => void) | null = null;
  ...
  public saveSettings(baseURL: string, apiKey: string, timeout: number, retries: number) {
  ...
  public onSettingsChange(callback: (settings: any) => void) {
  ...
  public getSettings() {
    ...
    this._cachedSettings = {
      baseURL: (config.get('baseURL') as string) || '', // Strict: No default localhost
      apiKey: (config.get('apiKey') as string) || '',
      timeout: (config.get('timeout') as number) || 30000,
      retries: (config.get('retries') as number) || 0
    };
